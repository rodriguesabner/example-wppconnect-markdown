...
import {FormatMarkdownService} from "wppconnect-markdown";

export default {
  name: 'App',
  mounted() {
    this.formatMarkdownService(this.$refs.message);
  ...
  watch:{
    messagesList: function(){
      this.handleChangeMessageList();
    }
  },
  methods: {
    handleChangeMessageList(){
      const lastIndex = this.messagesList.length - 1;
      this.formatMarkdownService(null, lastIndex);
    },
    sendMessage() {
